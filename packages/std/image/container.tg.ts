...
export const image = async (...args: std.Args<Arg>): Promise<tg.File> => {
	...
	// TODO - if workdir is set, make sure such a directory exists.
	...
	if (!envApplied) {
		// FIXME - validate there isn't already an entrypoint. What to do if there is?
		envApplied = true;
	...
	if (expose !== undefined) {
		// FIXME - fix the type!
		// config.config!.ExposedPorts = expose;
	}
	if (user !== undefined) {
