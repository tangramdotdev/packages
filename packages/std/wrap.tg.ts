...
export async function wrap(...args: std.Args<wrap.Arg>): Promise<tg.File> {
	...
	const buildToolchain = arg.buildToolchain
		...
		: std.triple.os(host) === "linux"
			...
			: await bootstrap.sdk.env(host); // FIXME - this is only true if build === host.
...
export namespace wrap {
	...
	export const arg = async (
		...
		// FIXME this detection would be better but breaks the linux cross case?
		// let host = host_;
		// if (host === undefined) {
