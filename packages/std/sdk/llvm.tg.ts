...
export const toolchain = async (arg?: LLVMArg) => {
	...
	if (!prebuilt_) {
		throw new Error("only prebuilt LLVM toolcahins are currently supported.");
	}
	if (prebuilt_) {
...
export const test = async () => {
	...
	const cOut = await $`
		set -x && clang -dumpmachine && clang -v -xc ${testCSource} -fuse-ld=lld -o $OUTPUT
	`
		.env(directory)
		.host(system)
		.then(tg.File.expect);
