...
export async function libc(unresolvedArg: tg.Unresolved<LibCArg>) {
...
function kindFromTriple(triple: string): LibcKind {
...
export async function linkerFlags(arg: LinkerFlagArg) {
	const host = arg.host ?? std.triple.host();
...
export async function constructSysroot(unresolvedArg: tg.Unresolved<LibCArg>) {
	const arg = await tg.resolve(unresolvedArg);
	const host = arg.host ?? std.triple.host();
