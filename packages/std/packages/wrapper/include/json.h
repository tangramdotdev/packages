...
enum {
	...
	ERROR_EXPECTED_CLOSING_QUOTE,
	ERROR_EXPECTED_CLOSING_BRACKET,
	ERROR_NUM_ERRRORS
};

const char* json_error_message(int code) {
	switch(code) {
		case ERROR_OK:
			return "ok";
		...
		case ERROR_EXPECTED_CLOSING_BRACE:
			return "missing closing ']'";
		case ERROR_EXPECTED_CLOSING_QUOTE:
			return "missing closing '\"'";
...
TG_VISIBILITY int parse_json_value (JsonParser* parser, JsonValue* value);
TG_VISIBILITY int parse_json_object (JsonParser* parser, JsonObject* object);
TG_VISIBILITY int parse_json_array (JsonParser* parser, JsonArray* value);
TG_VISIBILITY int parse_json_string (JsonParser* parser, String* value);
TG_VISIBILITY int parse_json_number (JsonParser* parser, double* value);
TG_VISIBILITY int parse_json_keyword (JsonParser* parser, JsonValue* value);
TG_VISIBILITY void print_json_value(JsonValue* value);
TG_VISIBILITY void print_json_object(JsonObject* object);
TG_VISIBILITY void print_json_array(JsonArray* array);
TG_VISIBILITY void print_json_string(String* string);
TG_VISIBILITY uint64_t json_array_len (JsonArray* array);
TG_VISIBILITY JsonValue* json_get (JsonObject* object, const char* k);
...
TG_VISIBILITY int parse_json_array (JsonParser* parser, JsonArray* array) {
	// Eat the leading '['.
	EAT_CHAR;
	...
	ENSURE_NOT_EMPTY;
	if (*parser->input.ptr != ']') {
		return ERROR_EXPECTED_CLOSING_BRACE;
	}
	EAT_CHAR;
	return ERROR_OK;
}

TG_VISIBILITY int parse_json_string (JsonParser* parser, String* string) {
	// Consume the opening '"'
	EAT_CHAR;
	...
	ENSURE_NOT_EMPTY;
	if (*parser->input.ptr != '"') {
		return ERROR_EXPECTED_CLOSING_QUOTE;
	}
	EAT_CHAR;
	return ERROR_OK;
}

TG_VISIBILITY bool is_digit (uint8_t tok) {
