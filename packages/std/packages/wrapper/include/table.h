...
TG_VISIBILITY int create_table (Arena*, Table*, size_t);
TG_VISIBILITY int insert (Arena*, Table*, String, String);
TG_VISIBILITY void remove (Table*, String);
TG_VISIBILITY String lookup (Table*, String);
TG_VISIBILITY String clookup (Table*, const char*);
TG_VISIBILITY void clear (Table*);
TG_VISIBILITY void print_table (Table*);

#ifdef TG_IMPLEMENTATION
TG_VISIBILITY int create_table (
	Arena* arena,
	Table* table,
	size_t capacity
) {
...
TG_VISIBILITY String lookup (Table* table, String key) {
	uint64_t hash	= fnv1a(key);
	Node* node	= table->list + hash % table->capacity;
	while(node) {
		if (streq(node->key, key)) {
...
TG_VISIBILITY String clookup (Table* table, const char* key) {
	String key_ = STRING_LITERAL(key);
	return lookup(table, key_);
}

TG_VISIBILITY void clear (Table* table) {
	Node* itr = table->list;
...
TG_VISIBILITY void print_table (Table* table) {
	Node* itr = table->list;
	Node* end = itr + table->capacity;
	for(; itr != end; itr++) {
		Node* node = itr;
