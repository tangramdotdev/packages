...
fn main() {
	// Setup tracing.
	#[cfg(feature = "tracing")]
	tangram_std::tracing::setup("TANGRAM_CODESIGN_PROXY_TRACING");
...
async fn run_proxy(
	...
	if let manifest::Executable::Path(artifact_path) = manifest.executable {
		...
		let signed_file_id = signed_file.id();
		#[cfg(feature = "tracing")]
		tracing::info!(?signed_file_id, "checked in the signed executable");
		...
		// Produce a new manifest with the signed executable, and the rest of the manifest unchanged.
		let new_manifest = Manifest {
			executable: manifest::Executable::Path(
				tangram_std::template_from_artifact(tg::Artifact::with_id(signed_file_id.into()))
					.to_data(),
			),
			..manifest
		};
...
impl Options {
	fn parse() -> tg::Result<Self> {
		...
		let mut passthrough = std::env::var("TANGRAM_CODESIGN_PROXY_PASSTHROUGH").is_ok();
		let codesign_program = std::env::var("TANGRAM_CODESIGN_COMMAND_PATH")
			.map_err(|error| tg::error!(source = error, "TANGRAM_CODESIGN_COMMAND_PATH not set"))?
			.into();
		let codesign_runtime_library_path =
			std::env::var("TANGRAM_CODESIGN_RUNTIME_LIBRARY_PATH").unwrap_or_default();
		...
		for (i, arg) in args.iter().enumerate() {
			...
			if arg.starts_with("--tg-") {
				...
				codesign_args.push(arg.clone());

				// Handle arguments that take a value (e.g., -s, --sign, -i, --identifier, etc.)
				if matches!(
					arg.as_str(),
					"-s" | "--sign"
						| "-i" | "--identifier"
						...
						| "--keychain" | "--digest-algorithm"
				) {
					// The next argument is the value for this flag
					if let Some(value) = args.get(i + 1) {
						codesign_args.push(value.clone());
						skip_next = true;
					}
				}
			} else {
				// If we haven't set the target yet, and this doesn't look like a flag value,
				// it's likely the target file.
				if codesign_target.is_none() {
					codesign_target = Some(arg.into());
				} else {
					// If we have already set the target, it might be an additional file or error.
					// For codesign, multiple files can be signed, but we'll handle the first one
					// as the primary target and pass others through as arguments.
					codesign_args.push(arg.clone());
				}
			}
		}
