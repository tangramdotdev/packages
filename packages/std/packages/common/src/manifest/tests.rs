...
#[tokio::test]
async fn inherit_from_file_preserves_dependency_and_wrapper_tokens() {
	...
	for permission in [None, Some(Node), Some(Subtree)] {
		...
		let mut expected = dependency_tokens;
		expected.inherit(&wrapper.state().tokens());
		...
		assert_eq!(
			tokens, expected,
			"dependency and wrapper tokens must both survive inheritance"
		);
	}
}
...
fn dependencies_merge_tokens_from_multiple_locations() {
	...
	tokens.insert(location.clone(), remote.state().tokens().local()[0].clone());
	...
	assert_eq!(tokens.local()[0].body.expires_at, 200);
	assert_eq!(tokens.get(&location)[0].body.expires_at, 100);
}
