...
int main(int argc, const char** argv) {
	...
	if (!segment) {
		...
		new_phdr_offset = ALIGN(output_size, 64);
		...
		for (int i = 0; i < ehdr->e_phnum; i++) {
			...
			TRACE("LOAD phdr[%d] %s", i, p_type_string(old_phdr[i].p_type));
		...
		for (int i = 0; i < ehdr->e_phnum; i++) {
			...
			TRACE("copying phdr[%d] %s", i, p_type_string(old_phdr[i].p_type));
			new_phdr[new_phdr_num] = old_phdr[i];
			new_phdr_num++;
		}
		TRACE("new_phdr_offset = %d", new_phdr_offset);
		TRACE("num phdrs: %d", new_phdr_num);		
	}
	
	// Find out where we're going to write the stub.
	size_t stub_offset = new_phdr 
		? ALIGN(new_phdr_offset + new_phdr_sz, 4096)
