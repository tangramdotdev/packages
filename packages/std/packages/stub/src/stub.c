...
static int read_executable (
	...
	ABORT_IF(
		pread64(fd, (void*)executable->section_headers, size, offset) != size,
	...
	ABORT_IF(
		pread64(fd, (void*)executable->section_string_table, size, offset) != size,
	...
	String TANGRAM_MANIFEST_SECTION_NAME = STRING_LITERAL(".note.tangram-manifest");
	for (; section_itr != section_end; section_itr++) {
		...
		if (streq(name, TANGRAM_MANIFEST_SECTION_NAME)) {
			...
			ABORT_IF(
				pread64(fd, (void*)data, size, offset) != size,
