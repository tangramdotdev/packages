...
VENDORCFG`;
	...
	// If cross-compiling, set linker and tool env vars, and tell cargo the
	// target triple so the user does not need to pass --target manually.
	if (crossCompiling) {
	...
	// detect workspace members and route them to passthrough (direct rustc
	// invocation). Without this, all crates would be routed through the proxy
	// sandbox since cargo sets cwd to each crate's source directory.
	...
	if (proxy) {
		...
		// Resolve an SDK for the sandbox so that proc-macros, build scripts,
		// and other crate types requiring a linker can be compiled inside the
		// Tangram sandbox rather than being passthrough'd uncached.
...
env | cut -d= -f1 > "$TGRUSTC_HOST_ENV_SNAPSHOT"`;
	...
	// Build feature flags to inject after the cargo subcommand.
...
linker = "${hostLinker}"`;
	...
	// Set the TGRUSTC_SOURCE_DIR variable.
