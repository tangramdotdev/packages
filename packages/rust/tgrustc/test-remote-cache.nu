...
# Test remote pull caching for tgrustc in the `tg run` (host cargo) flow.
#
# Cargo runs on the host with `RUSTC_WRAPPER=tgrustc`. Workspace members
...
def main [
    ...
    # Step 3: Run cargo build on primary. Set TGRUSTC_SPAWN_LOG so the
    ...
    bash -c $"cd ($workspace_dir) && TGRUSTC_SPAWN_LOG=($spawn_log) tangram run ($cmd_id) -- build 2>&1 | tee ($baseline_log)" o+e>| ignore
    ...
    if ($orphan_rustc | length) < 1 {
        error make { msg: "no sandbox spawns recorded; check TGRUSTC_SPAWN_LOG plumbing" }
    }
    ...
    let fresh_run = (bash -c $"cd ($workspace_dir) && TANGRAM_URL=http://localhost:($fresh_port) TGRUSTC_SPAWN_LOG=($fresh_spawn_log) tangram run ($fresh_cmd_id) -- build 2>&1 | tee ($rebuild_log); exit ${PIPESTATUS[0]}" | complete)
