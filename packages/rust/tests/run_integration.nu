...
# Assert deps/ contains .rlib files that are writable regular files (copies from proxy).
# In tg run mode, run_proxy produces writable copies so that cargo
# can fingerprint them correctly and overwrite them on subsequent builds.
def assert_mixed_dep_types [profile: string] {
    ...
    # All files should be regular writable copies in tg run mode.
...
# Check binary presence and optionally verify output and/or successful execution.
...
# Extract cargo's self-reported build time from the "Finished" line in stderr.
...
# Print timing breakdown: total wall time, cargo self-reported time, and overhead.
